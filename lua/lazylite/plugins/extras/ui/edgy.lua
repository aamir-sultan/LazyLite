return {
  -- edgy
  {
    ...
    opts = function()
      ...
      if LazyLite.has("neo-tree.nvim") then
        local pos = {
          filesystem = "left",
          buffers = "top",
          git_status = "right",
        ...
        local sources = LazyLite.opts("neo-tree.nvim").sources or {}
        for i, v in ipairs(sources) do
          table.insert(opts.left, i, {
            ...
            pinned = true,
            open = function()
              vim.cmd(("Neotree show position=%s %s dir=%s"):format(pos[v] or "bottom", v, LazyLite.root()))
            end,
          })
        end
      end
