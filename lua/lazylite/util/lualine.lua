...
function M.status(icon, status)
  ...
  return {
    ...
    color = function()
      return LazyLite.ui.fg(colors[status()] or colors.ok)
    end,
  }
end
...
function M.cmp_source(name, icon)
  icon = icon or LazyLite.config.icons.kinds[name:sub(1, 1):upper() .. name:sub(2)]
  local started = false
...
function M.pretty_path(opts)
  ...
  return function(self)
    ...
    local root = LazyLite.root.get({ normalize = true })
    local cwd = LazyLite.root.cwd()
