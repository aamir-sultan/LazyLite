...
function M.detectors.lsp(buf)
  ...
  local clients = LazyLite.lsp.get_clients({ bufnr = buf })
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  LazyLite.info(lines, { title = "LazyLite Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    LazyLite.root.info()
  end, { desc = "LazyLite roots for the current buffer" })
  ...
  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged", "BufEnter" }, {
    group = vim.api.nvim_create_augroup("lazylite_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return LazyLite.is_win() and ret:gsub("/", "\\") or ret
end
