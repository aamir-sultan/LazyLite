...
function M.decorate(name, mod)
  ...
  setmetatable(mod, {
    __call = function(_, ...)
      ...
      LazyLite.deprecate("LazyLite." .. name, to)
    ...
    __index = function(_, k)
      if M.moved[name][k] then
        ...
        LazyLite.deprecate("LazyLite." .. name .. "." .. k, to)
        local ret = vim.tbl_get(_G, unpack(vim.split(to, ".", { plain = true })))
        return ret
      end
      return nil
...
function M.lazygit()
  LazyLite.deprecate("LazyLite.lazygit", "Snacks.lazygit")
  return Snacks.lazygit
end

function M.toggle()
  LazyLite.deprecate("LazyLite.toggle", "Snacks.toggle")
  return {
    map = function() end,
    wrap = function()
      return {}
