...
function M.formatexpr()
  if LazyLite.has("conform.nvim") then
...
function M.info(buf)
  ...
  LazyLite[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    LazyLite.warn("No formatter available", { title = "LazyLite" })
  end
end
...
function M.health()
  ...
  local has_extra = vim.tbl_contains(Config.spec.modules, "lazylite.plugins.extras.lsp.none-ls")
  if has_plugin and not has_extra then
    LazyLite.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in LazyLite.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `lazylite.plugins.extras.lsp.none-ls` extra,",
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return LazyLite.format.enabled()
    end,
    set = function(state)
      LazyLite.format.enable(state, buf)
    end,
  })
end
